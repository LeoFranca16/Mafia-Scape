...
public class PlayerMOV : MonoBehaviour
{
    ...
    void Update()
	{
		...
		if (Input.GetKeyDown ("down")) {
			...
			if (position.y < Map.mapHeight && Map.tiles [(int)(position.y + 1), (int)position.x] != 0)
				position.y++;
        ...
        //seta cima
			...
			if (position.y > 0 && Map.tiles [(int)(position.y - 1), (int)position.x] != 0)
				position.y--;
        ...
        //seta esquerda
			...
			if (position.x > 0 && Map.tiles [(int)position.y, (int)(position.x - 1)] != 0)
				position.x--;
        ...
        //seta direita
			...
			if (position.x < Map.mapWidth && Map.tiles [(int)position.y, (int)(position.x + 1)] != 0)
				position.x++;
